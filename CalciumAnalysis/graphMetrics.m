...
% LAST UPDATED: 12/09/2019  Derived from CalculateNetworkMetrics
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
function [GlobalMetrics, LocalMetrics, AdjacencyMatrix, MaskBoundaries, ...
    ...
    graphMetrics(Masks, AdjacencyMatrix)
...
if nNodes > 5000
    ...
    fprintf('[graphMetrics] nEdges=%d\n', nEdges);
    
    % stopping processing if number of edges > 10000
    if nEdges > 20000 %10000
        processCompleted = false;
        GlobalMetrics = 0;
        LocalMetrics = 0;
    ...
    elseif returnTotalVariableSize(whos) > 16
        processCompleted = false;
fprintf('[graphMetrics] returnTotalVariableSize(whos)=%d\n', returnTotalVariableSize(whos));
