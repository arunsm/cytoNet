...
SIB = ComputeSIB(stainImage,nuMask,numOfCell,numOfStain,aDiv,rDiv,set_stain,row,col, data.cellmaskdata);
...
function SIB = ComputeSIB(stainImage,nuMask,numOfCell,numOfStain,aDiv,rDiv,set_stain,row,col, cellMask)

theta = linspace(eps,pi-(eps),round(aDiv/2));
nTheta = length(theta);
maxfractionaldelta = 0;
parfor idx_cell = 1:numOfCell
    tic
    fprintf('Processing cell #%g:\n',idx_cell)
    
    cytoMask = (cellMask{idx_cell} & ~nuMask{idx_cell}) | bwperim(nuMask{idx_cell});
...
%         Y2 = tan(theta(idx_angle))*(X2-xc)+yc;
        ...
        [inXcy inYcy] = ext_lineinmask(cytoMask,X,Y);
    ...
    for idx_stain = set_stain
        for idx_band = 1:rDiv
            ...
            intensity = bandMask.*(cytoMask).*(stainImage{idx_stain});
            intensityAvg(idx_stain,idx_band) = sum(intensity(:)) / numBandPixels;
        end
    end
    tmpintensityavg{idx_cell} = intensityAvg;    
    
    time = toc;
    fprintf('Elapsed time is %3.1f seconds.\n\n',time)
end

SIB.intensityavg = tmpintensityavg;
